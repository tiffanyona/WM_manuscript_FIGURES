...
save_path = 'C:/Users/Tiffany/Google Drive/WORKING_MEMORY/PAPER/Panel figures/Fig. 2 Model//'
cm = 1/2.54
...
for regressor, panel, color in zip(['pi', 't11','t22'],[a, a, a], ['darkgreen','grey','grey']):
    if regressor == 'pi':
        ...
        sns.scatterplot(x=xA,y=regressor,data=full_fit, ax=panel, color=color, style='delay',  legend=False, alpha=0.7, size=1)
...
sns.violinplot(data=plot, palette=['darkgreen','grey','grey'],ax=panel, width=0.5)
sns.violinplot(data=plot, palette=['darkgreen','grey','grey'],ax=panel, width=0.5, linewidth=0)
panel.set_xlabel('pi         t11         t22')

# ----------------------------------------------------------------------------------------------------------------
# -----------------############################## E Panel - X-Y #################################-----------------------
panel=b

df_results = pd.read_csv(save_path+'X-Y.csv')
...
panel=d1
df_results = pd.read_csv(save_path+'after_correct.csv')
...
panel=e1
df_results = pd.read_csv(save_path+'previous_correct.csv')
...
cumulative_autocorrelation_hit_model=pd.read_csv('hit_autocorrelation_10_model_V2.csv', index_col=0)
cumulative_autocorrelation_repeat_model=pd.read_csv('repeat_autocorrelation_10_model_V2.csv', index_col=0)

cumulative_autocorrelation_hit_data =pd.read_csv('hit_autocorrelation.csv', index_col=0)
cumulative_autocorrelation_repeat_data =pd.read_csv('repeat_autocorrelation.csv', index_col=0)

panel = f
color = 'darkgreen'
corr = cumulative_autocorrelation_hit_model.mean(axis=1)

panel.plot(np.arange(1,len(corr)+1), corr ,marker='', color=color)
...
panel.plot(np.arange(1,len(corr)+1), mean ,marker='', color='grey')
...
save_path = 'C:/Users/Tiffany/Google Drive/WORKING_MEMORY/PAPER/Panel figures/Fig. 2 Model/Supp. 2.1. Extra results from model/'
plt.savefig(save_path+'/Fig. Supp. 2.1. Extra results from model_v2.svg', bbox_inches='tight',dpi=300)
plt.show()