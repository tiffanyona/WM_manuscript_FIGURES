...
def plot_decoder(left, df_cum_sti, df_shuffle, baseline=0.5,individual_sessions=False, colors=['black'], upper_limit=0.2, variables_combined=['WM_roll_1']):
    for color, variable,left in zip(colors,variables_combined,[left]):
        ...
        df_for_boots = df_cum_sti.loc[:, (df_cum_sti.columns != 'session_shuffle')
                                       &(df_cum_sti.columns != 'fold')].groupby('session').mean(numeric_only=True).reset_index()
        ...
        lower =  df_cum_sti.select_dtypes('number').quantile(0.025)
        upper =  df_cum_sti.select_dtypes('number').quantile(0.975)
...
for epoch in df_animal_shuffle.train.unique():
    ...
    df_shuffle_mean[epoch] = df_animal_shuffle.loc[(df_animal_shuffle.train == epoch)].groupby(['times']).mean(numeric_only=True).drop(columns='fold').mean(axis=1).values
df_shuffle_mean.index = df_animal_shuffle.groupby('times').mean(numeric_only=True).index

df_new = df_animal_sti.loc[:, df_animal_sti.columns != 'fold'].groupby(['subject','train']).mean(numeric_only=True)
df_new.reset_index(inplace=True)
df_new = df_new.groupby('train').mean(numeric_only=True)
...
_xlabels = ["-2",'',"","","","","","","0",'',"","","","","","","2","","","","","","","","4","","","","","","","","6","","","","","","","","8"]
panel.set_xticks(range(len(df_new.columns)))
panel.set_xticklabels(_xlabels[:len(df_new.columns)])
_ylabels = ["-2",'',"","","0",'',"","","2","","","","4","","","","6","","","","8"]
panel.set_yticks(range(len(df_new.index)))
panel.set_yticklabels((_ylabels + ['']*len(df_new.index))[:len(df_new.index)])
...
# session = pd.read_csv(DATA_DIR / 'E20_2022-02-14_16-01-30.csv', sep=',',index_col=0)
...
df_temp = pd.read_csv(path+file_name+'.csv', index_col=0).reset_index(drop=True)

panel = g1
# color_list = ['lightblue','darkgrey', 'blue','lightgrey','grey']
_n_cond = df_temp['condition'].nunique()
color_list = ['darkgrey'] * _n_cond
orderlist=['Stim x Late Delay','Late Delay x Response','Early x Late Delay','Late Delay x Late Delay*',]

sns.stripplot(x='condition',y='vector', hue='condition', legend=False, data=df_temp, order=orderlist, jitter=0.2, size=3, palette=color_list, edgecolor='white', ax=panel, linewidth=0.1)
...
sns.boxplot(x='condition',y='vector', hue='condition', legend=False, data=df_temp, order=orderlist, palette=['black'] * _n_cond, ax=panel, width=0.4)
...
#     for color, variable in zip(colors,variables_combined):
#         real = np.array(np.mean(df_sti.loc[(df_sti['trial_type'] == variable)].groupby('session').median(numeric_only=True).drop(columns=['fold','delay','score']).dropna(axis=1)))
...
#         df_new = df_iter.loc[(df_iter.trial_type==variable)].drop(columns=['fold', 'delay','session']).groupby('times').mean(numeric_only=True)
